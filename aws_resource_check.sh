...
SUBNET_ID=$(aws ec2 describe-security-groups   --filters Name=tag:Name,Values=sample_security_group --query "SecurityGroups[*].GroupId" --output text 2> /dev/null)
  if [ "$SUBNET_ID" != "None" ]; then
    echo "既存のセキュリティグループ($SUBNET_ID)をimportします"
    terraform import aws_security_group.sample_security_group $SUBNET_ID
  else
    echo "subnetがないので作成します"
  fi